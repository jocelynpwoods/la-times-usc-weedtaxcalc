...
$(document).ready(function(){ // begin document.ready block
	    ...
	    $('#input').change(function(){
	    	...
	    	var whatever = $("#input").val();

	    	leng = weedtax.length;

	    	for (var i = 0; i < leng; i++) {

		    	if (whatever != weedtax[i].city) {
					//$('.setme').html('<p>Sorry, we do not have data for ' + whatever + '.</p>');
					$('.setme').html('<p>Sorry, that city is not in our list.</p>');
		    	} else {
